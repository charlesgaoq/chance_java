...
public class LoginUtil {
    ...
    private static final String LOGIN_FAILED_CAUSE_PASSWORD_WRONG = "(wrong password) keytab "
            + "file and user not match, you can kinit -k -t keytab user in client server to check";

    private static final String LOGIN_FAILED_CAUSE_TIME_WRONG = "(clock skew) time of local "
            + "server and remote server not match, please check ntp to remote server";

    private static final String LOGIN_FAILED_CAUSE_AES256_WRONG = "(aes256 not support) "
            + "aes256 not support by default jdk/jre, need copy local_policy.jar and "
            + "US_export_policy.jar from remote server in path "
            + "/opt/huawei/Bigdata/jdk/jre/lib/security";

    private static final String LOGIN_FAILED_CAUSE_PRINCIPAL_WRONG = "(no rule) principal "
            + "format not support by default, need add property "
            + "hadoop.security.auth_to_local(in core-site.xml) value RULE:[1:$1] RULE:[2:$1]";

    private static final String LOGIN_FAILED_CAUSE_TIME_OUT = "(time out) can not connect to "
            + "kdc server or there is fire wall in the network";

    private static final boolean IS_IBM_JDK = System.getProperty("java.vendor").contains("IBM");

    public static synchronized void login(String userPrincipal, String userKeytabPath,
        ...
        if (!krb5ConfFile.exists()) {
            ...
            throw new IOException(
                    "krb5ConfFile(" + krb5ConfFile.getAbsolutePath() + ") does not exsit.");
        }
        if (!krb5ConfFile.isFile()) {
            LOG.error("krb5ConfFile(" + krb5ConfFile.getAbsolutePath() + ") is not a file.");
            throw new IOException(
                    "krb5ConfFile(" + krb5ConfFile.getAbsolutePath() + ") is not a file.");
        }
    ...
    // throw new IOException("UserGroupInformation is not SecurityEnabled,"
    // + " please check if core-site.xml exists in classpath.");
    // }
    ...
    // + "has logined. please check your enviroment , especially "
    // + "when it used IBM JDK or kerberos for OS count login!!");
    ...
    private static void setKrb5Config(String krb5ConfFile) throws IOException {
        ...
        if (!ret.equals(krb5ConfFile)) {
            ...
            throw new IOException(
                    JAVA_SECURITY_KRB5_CONF_KEY + " is " + ret + " is not " + krb5ConfFile + ".");
        }
    }
    ...
    public static void setZookeeperServerPrincipal(String zkServerPrincipalKey,
        ...
        if (!ret.equals(zkServerPrincipal)) {
            ...
            throw new IOException(
                    zkServerPrincipalKey + " is " + ret + " is not " + zkServerPrincipal + ".");
        }
    }
    ...
    // throw new IOException(
    // "current user is " + currentUser + ", but loginUser is " + loginUser + ".");
    // }
    // if (!loginUser.hasKerberosCredentials()) {
    // LOG.error("current user is " + currentUser + " has no Kerberos Credentials.");
    // throw new IOException(
    // "current user is " + currentUser + " has no Kerberos Credentials.");
    // }
    ...
    /**
     * copy from hbase zkutil 0.94&0.98 A JAAS configuration that defines the login modules that we
     * want to use for
     * login.
     */
    private static class JaasConfiguration extends javax.security.auth.login.Configuration {
        ...
        private static final Map<String, String> KB = new HashMap<String, String>();

        static {
            if (IS_IBM_JDK) {
                KB.put("credsType", "both");
            } else {
                KB.put("useKeyTab", "true");
                KB.put("useTicketCache", "false");
                KB.put("doNotPrompt", "true");
                KB.put("storeKey", "true");
            }

            KB.putAll(BASIC_JAAS_OPTIONS);
        }

        private static final AppConfigurationEntry KB_LOGIN = new AppConfigurationEntry(
                KerberosUtil.getKrb5LoginModuleName(), LoginModuleControlFlag.REQUIRED, KB);

        private static final AppConfigurationEntry[] KBL_CONF = new AppConfigurationEntry[] {
            KB_LOGIN };
        ...
        private JaasConfiguration(String loginContextName, String principal, String keytabFile,
            ...
            initKerberosOption();
            LOG.info("JaasConfiguration loginContextName=" + loginContextName + " principal="
                    + principal + " useTicketCache=" + useTicketCache + " keytabFile="
                    + keytabFile);
        }
        ...
        private void initKerberosOption() throws IOException {
            if (!useTicketCache) {
                if (IS_IBM_JDK) {
                    KB.put("useKeytab", keytabFile);
                } else {
                    KB.put("keyTab", keytabFile);
                    KB.put("useKeyTab", "true");
                    KB.put("useTicketCache", useTicketCache ? "true" : "false");
                }
            }
            KB.put("principal", principal);
        }
